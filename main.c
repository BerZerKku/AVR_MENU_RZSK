...
strCorrParam sCorrParam[2]; 					//хранение выводимых на экран коррекций тока и напряжения
...
/// Структура элементов пункта Меню / Управление
strControl stControl;

static void vControlMenuSetup(void);
static void FuncInpCorrParam(void);

/** Формирование пункта Меню / Управление
 *	Зависит от наличия наладочного пуска.
 */
static void vControlMenuSetup(void)
{
	uint8_t cnt = 0;
	...
	// Сброс своего
	stControl.fuName[cnt] 			= fuControlItem0;
	stControl.uNumber[cnt] 			= 1;
	stControl.eActionNumber[cnt] 	= CTRL_RESET;
	cnt++;
	...
	{
		// включение
		stControl.fuName[cnt] 		= fuControlItem1;
		stControl.uNumber[cnt] 		= 8;
		stControl.eActionNumber[cnt]= CTRL_START;
	}
	cnt++;
	
	stControl.uItemQuantity = cnt;
}
...
// ВВод коррекции тока и напряжения
static void FuncInpCorrParam(void)
{
	int16_t ii;
	
	//выводим на экран "Ввод:" и очищаем строку до конца
	LCDprintf(4, 1, 2, MenuInputData, 1); 
	FuncClearCharLCD(4, 6, 15); 	
	...
	if ((PressKey == 'D') && (NumberInputChar > 0)) {
		if (InputParameter == 1) {
			if (NumberInputChar != 0) {
				NumberInputChar--;
				// для напряжения прыгаем через точку
				if (NumberInputChar == 2) 
					NumberInputChar = 1;
			}
			InputValue[NumberInputChar] = '0';
		} else {
			InputValue[--NumberInputChar] = '0';
		}
	}
	...
	if (PressKey=='E') {
		if (InputParameter == 1) { 
			...
			if (ii==0) {
				//если ввели 0, то считаем это командой сброса коррекции
				InputParameter = 4;
			} else {
				ii = ii - UlineValue + sCorrParam[COR_VOLTAGE].Corr;
				TrValueD = (uint8_t) ((int8_t) (ii/10));
				TrValueD <<= 8;
				TrValueD += (uint8_t) ((int8_t) (ii%10)*10);
			}
		} else if (InputParameter == 2) {
			ii = 0;
			if (NumberInputChar==3){
				ii = ((int16_t) (InputValue[0]&0x0F)*100 + (InputValue[1]&0x0F)*10+ (InputValue[2]&0x0F));
			} else if (NumberInputChar==2) {
				ii = ((int16_t) (InputValue[0]&0x0F)*10+ (InputValue[1]&0x0F));
			} else if (NumberInputChar==1) {
				ii = (int16_t) (InputValue[0]&0x0F);
			} else {
				ii = IlineValue;
			}
			...
			if (ii==0) {
				...
				ii = ii - IlineValue + sCorrParam[COR_CURRENT].Corr;
...
static void FuncPressKey(void)
{
	...
	else if (MenuLevel == LVL_MENU)
	{
		switch(PressKey)
		{
			...
			case '3':		// Переход в пункт "Управление"
			{
				LCDbufClear();
				LCD2new = 1;
				MenuLevel = LVL_CONTROL;
				ShiftMenu = 0;
				MaxShiftMenu = 0;
				MaxDisplayLine = 3;
			}break;
	...
	else if (MenuLevel == LVL_CONTROL)
	{
		switch(PressKey)
		{
			...
			case 'D':
			{
				...
				if (stControl.uItemQuantity > MaxDisplayLine)
				{
					if (ShiftMenu < (stControl.uItemQuantity - MaxDisplayLine))
					{
						ShiftMenu++;
						LCD2new = 1;
					}
			...
			default:
			{
				uint8_t key = PressKey - '1';
				
				if (key < stControl.uItemQuantity)
				{
					SelectValue = 0x72;
					TrValue = stControl.uNumber[key];
					InputSelectValue = MaxValue = MinValue = (uint8_t) stControl.eActionNumber[key];
					MassSelectValue = fuControlAction;
					WorkRate = 2;
				}
			}
		}
						...
						switch(MenuLevel)
						{
							...
							case LVL_SETUP_GLB:
							{
								switch(TrParam)
								{
									...
									case 10:
										Tr_buf_data_uart[4] = NumberTransCom;
										Tr_buf_data_uart[5] = Hi(TrValueD);
										Tr_buf_data_uart[6] = Low(TrValueD);
										TransDataInf(0xB3, 3);
										break;
									default: 
										TransDataByte(0xB5 + TrParam, TrValue);
								}
							}break;
							case LVL_TEST:
							{
			...
			switch (MenuLevel)
			{
				...
				case LVL_SETUP_GLB: 
				{
					if (LCD2new==1)
					{
						...
						if (ValueVsRange==0)
						{
							...
							switch(ShiftMenu){
								case 0: 
									LCDprintf(3,11,2,MenuAllSynchrTimerNum[MenuAllKeepComPRM],1); 
									break;
								case 1: 
									LCDprintf(3,11,2,MenuAllSynchrTimerNum[MenuAllKeepComPRD],1); 
									break;
								case 2: 
									LCDprint(3,11,2,MenuAllTimeRerun,1); 
									break;
								case 3: 
									LCDprintf(3,11,2,MenuAllSynchrTimerNum[MenuAllControlUout],1); 
									break;
								case 4: 
									LCDprint(3,11,2,MenuAllCF,1); 
									break;
								case 5: 
									LCDprintChar(3,11,'-');
									LCDprint(3,12,2,MenuVoltageLimit1,1);
									break;
								case 6: 
									LCDprint(3, 11, 2, sCorrParam[COR_VOLTAGE].Print, 1); 
									break;
								case 7: 
									LCDprint(3, 11, 2, sCorrParam[COR_CURRENT].Print, 1);
									break;
							}
						}
						else
				...
				case LVL_CONTROL:
				{
					if (LCD2new == 1)
					{
						...
						// Если идет ввод параметра, выйдем
						if (WorkRate != 0)
							break;
						
						vControlMenuSetup();
						
						for(unsigned char i = 0, cnt; i < 3; i++)
						{
							cnt = i + ShiftMenu;
							if (cnt >= stControl.uItemQuantity)
							{
								FuncClearCharLCD(i + 2, 1, 20);
							}
							else
							{
								LCDprintChar(i + 2, 1, cnt + '1');
								LCDprintf(i + 2, 2, 2, stControl.fuName[cnt], 1);
							}
						}
